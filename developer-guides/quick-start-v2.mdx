...
<Tabs>
  <Tab title="Method 1">
    ...
    2. Make a request to the [`POST /v2/team/datasets/{id}/datasources`](/api-reference/create-data-source) endpoint. Replace the `id` value with the ID of the dataset you've created in the previous sub-step.
    ...
    When making the request, specify either `url` or `fileKey`, **but not both**. Use `url` to upload a file through a publicly accessible URL. For privately accessible files, use `fileKey`.
        ...
        **Example request**:
        ...
        ```curl cURL
        curl --request POST \
          --url https://ai.data.cloud/api/v2/team/datasets/{datasetId}/datasources \
          --header 'Content-Type: application/json' \
          --header 'x-pd-api-key: $PD_API_KEY' \
          --data '{
          "name": "test.csv",
        ...
        url = "https://ai.data.cloud/api/v2/team/datasets/{id}/datasources"
