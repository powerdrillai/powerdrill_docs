---
title: "Overview"
description: "Overview of Powerdrill Enterprise API endpoints and capabilities"
---

<CardGroup cols={2}>
<Card 
    title="Sessions" 
    icon="comment-dots" 
    href="/api-reference/create-sessions"
    >
    Create and manage your sessions to converse with your data.
</Card>
<Card 
    title="Datasets" 
    icon="database" 
    href="/api-reference/create-dataset"
    >
    Use datasets are your knowledge bases, bringing AI to your data.
</Card>
<Card 
    title="Data Sources" 
    icon="file-lines" 
    href="/api-reference/delete-data-source"
    >
    Manage your data sources by leveraging advanced techniques such as offline indexing, vector storage and retrieval.
</Card>
<Card 
    title="Jobs" 
    icon="bag-shopping" 
    href="/api-reference/create-job"
    >
    Run a job to start analyzing your data.
</Card>
<Card 
    title="Agents" 
    icon="user" 
    href="/api-reference/create-agent"
    >
    Create and manage data agents to tackle complex, expert-level data analysis tasks.
</Card>
<Card 
    title="Subscriptions" 
    icon="user" 
    href="/api-reference/subscriptions"
    >
    Choose from a variety of plans that suit your needs.
</Card>
</CardGroup>