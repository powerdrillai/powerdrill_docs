...
Below is a Python example of a streaming request:
...
id:0ca99d5d-20cf-416c-8bda-0b2549dc8733
event:CODE
data:{"id":"0ca99d5d-20cf-416c-8bda-0b2549dc8733","model":"","choices":[{"delta":{"content":"```python\n\nimport pandas as pd\n\ndef invoke(input_0: pd.DataFrame) -> pd.DataFrame:\n    '''\n    input_0: pd.DataFrame  makeovermonday-a-century-of-global-deaths-from-disasters_decadal-deaths-disasters-type.csv\n    '''\n    # Group by 'Year' and sum the deaths for each disaster type\n    aggregated_data = input_0.groupby('Year').sum()\n    \n    # Select only the columns related to deaths\n    death_columns = [\n        'Deaths - Drought (decadal)', \n        'Deaths - Flood (decadal)', \n        'Deaths - Earthquake (decadal)', \n        'Deaths - Extreme weather (decadal)', \n        'Deaths - Extreme temperature (decadal)', \n        'Deaths - Volcanic activity (decadal)', \n        'Deaths - Wildfire (decadal)', \n        'Deaths - Glacial lake outburst flood (decadal)', \n        'Deaths - Dry mass movement (decadal)', \n        'Deaths - Wet mass movement (decadal)', \n        'Deaths - Fog (decadal)'\n    ]\n    \n    # Filter the aggregated data to include only the death columns\n    output = aggregated_data[death_columns].reset_index()\n    \n    return output\n\n```"},"index":0}],"created":1731664185,"groupId":"0ca99d5d-20cf-416c-8bda-0b2549dc8733","groupName":"Aggregate the data by year and calculate the total number of deaths for each disaster type across all entities. This will prepare the data for visualization.","stage":"Analyze"}

id:0ca99d5d-20cf-416c-8bda-0b2549dc8733
event:TABLE
data:{"id":"0ca99d5d-20cf-416c-8bda-0b2549dc8733","model":"","choices":[{"delta":{"content":{"name":"aggregated_data.csv","url":"https://static.powerdrill.ai/tmp_datasource_cache/code_result/cm37bchx106e301l1v9yf67yc/42b06cab-f91b-461b-bc72-40509bad07ad.csv","expiredAt":"2024-11-21T09:49:45.773535Z"}},"index":0}],"created":1731664185,"groupId":"0ca99d5d-20cf-416c-8bda-0b2549dc8733","groupName":"Aggregate the data by year and calculate the total number of deaths for each disaster type across all entities. This will prepare the data for visualization.","stage":"Analyze"}
...
The response is returned as multiple response blocks. Each `data` object is a response block.

The fields contained in the `data` object include:

- `content`: The block content.

- `created`: The content creation timestamp.
...
- `groupName`: The name of the group, for example, `Conclusions`.

- `stage`: The stage to which the response block belongs. Two stages are available:  
  - `Analyze`
  - `Respond`

However, if streaming is disabled, Powerdrill will return the response once the entire response is ready. 

Let's use the same request as an example. The only difference is that we set `stream` to `False`.
...
- `blocks`: a list of response blocks that together form the entire response. Each block has the following properties:
  - `type`: Indicates the block's content type. Possible values include:

    - `MESSAGE`: Represents a piece of text.

    - `CODE`: Represents a code snippet returned in Markdown format.
    - `TABLE`: Represents tabular content, including:
      - `name`: The `.csv` file name.
      - `url`: The S3 key or URL to the file.
      - `expiredAt`: The expiration time for `url`.
    - `IMAGE`: Represents image content, including:
      - `name`: The image name.
      - `url`: The S3 key or URL to the image.
      - `expiredAt`: The expiration time for `url`.
    - `SOURCES`: Represents the source of the response block, including:
      - `source`: The file name of the data source.
      - **`datasourceId`**: The ID of the data source.
      - **`datasetId`**: The ID of the dataset.
      - **`fileType`**: The name extension of the data source file.

    - **`QUESTIONS`**: Represents follow-up questions suggested by Powerdrill. 
