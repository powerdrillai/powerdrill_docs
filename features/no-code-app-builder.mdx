---
title: No-Code App Builder
description: 'Offers a no-code approach to building Apps by using natural language'
---

## Introduction

The No-Code App Builder feature is a no-code approach that allows you to build Apps by using natural language. With this feature, all you need is to provide an App name and a piece of prompt written in natural language, and then your App is right here, ready to use. 

The No-Code App Builder supports two types of Apps:

 - **Text generators**: can generate high-quality text in response to given prompts.

 - **Chatbots**: are capable of providing accurate responses to inqueries.

## How to use this feature

The following example describes how to create a text generator on Powerdrill.

### Prerequisites

You have signed in to Powerdrill. 

For more information about how to sign in to Powerdrill, see Step 1 in [Try Out Powerdrill in 3 Minutes](/quick-start#step-1-sign-in-to-powerdrill).

### Procedure

1. On the **Apps** page, click **+ App** in the upper-right corner.
2. In the **Create App** dialog box, enter the App name, choose the type, and click **Continue**.
3. On the **Prompt Setup** page, configure related information.

    If you set the type **Text Generator**, parameters needed include:

    | Parameter | Description |
    | :- | :- |
    | Prompt | Used to guide your App's response. Click **Save** after setting it. |
    | Variables | The elements you specify in your prompts in the format of "{\{\variable_name}\}\". Variables are fields that must be specified when using the App. The variables are automatically generated based on your prompt. |
    | Datasets | If you want your App to reference specific knowledge when generating content, click **+ Select** to associate the App with selected datasets. For information about how to create a dataset, see Step 2 in [Try Out Powerdrill in 3 Minutes](/quick-start#step-2-import-your-data). |

    <br/>


    If you set the type **Chatbot**, parameters needed include:    

    | Parameter | Description |
    | :- | :- |
    | Prompt | Used to set your App's behavior. Click **Save** after setting it. |
    | Welcome Message | The initial greeting displayed at the start of a conversation. |
    | Datasets | If you want your App to reference specific knowledge when generating content, click **+ Select** to associate the App with selected datasets. For information about how to create a dataset, see Step 2 in [Try Out Powerdrill in 3 Minutes](/quick-start#step-2-import-your-data). |

    <br/>    

4. Click **Finish**. 

Now, your App is ready to use.


### More actions

The execution results of your App are recorded as output history. You can click the button next to **Reset** to view and manage them.

If you're not satisfied with the variables you specify, you can click **Reset** to reset all variables to the default settings.

You can click the **Settings** tab of your App to modify your Apps settings, including the name, description, and prompt setup.

