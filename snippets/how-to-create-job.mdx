...
For each new organization, Powerdrill Enterprise offers 500 general jobs for free. After the free quota is consumed, to run more general jobs, you must subscribe to Powerdrill Enterprise via the [POST /v1/team/subscriptions](create-subscription) endpoint. 

**Sample request**:

<CodeGroup>
```curl cURL
curl --location 'https://ai.data.cloud/api/v1/team/subscriptions' \
--header 'x-pd-api-key: $PD_API_KEY' \
--header 'Content-Type: application/json' \
--data '{
...
url = "https://ai.data.cloud/api/v1/team/subscriptions"
...
To check whether your subscription is active, make a request to the [`GET /v1/team/subscriptions/{subscriptionId}`](get-subscription) endpoint.
...
<CodeGroup>
```curl cURL
curl --location 'https://ai.data.cloud/api/v1/team/subscriptions/sub-cfgdsagdgsfa45232' \
--header 'x-pd-api-key: $PD_API_KEY' \
--data ''
```
...
url = "https://ai.data.cloud/api/v1/team/subscriptions/sub-cfgdsagdgsfa45232"
